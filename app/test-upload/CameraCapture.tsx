        ...
        */}
        {isVideoVisible && (
          <div className="relative">
            <video
              ...
              playsInline
              muted
              className={`w-full rounded-lg border border-amber-200 dark:border-zinc-600 bg-black ${
                cameraState.status === "loading" ? "invisible" : ""
              }`}
            />
            {/* Loading overlay shown on top of the (invisible) video */}
            {cameraState.status === "loading" && (
              <div className="absolute inset-0 rounded-lg bg-amber-100/80 dark:bg-zinc-700/80 flex items-center justify-center text-amber-800 dark:text-amber-200 text-sm">
                Starting camera…
              </div>
            )}
          </div>
        ...
        {previewUrl && (
          <img
            src={previewUrl}
            alt="Captured photo"
            className="w-full rounded-lg border border-amber-200 dark:border-zinc-600"
          />
        )}

        {/* Idle state — no camera, no preview */}
        {cameraState.status === "idle" && !previewUrl && (
