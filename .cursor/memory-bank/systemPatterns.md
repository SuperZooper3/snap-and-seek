...
4. Radar: initialData has distanceMeters; on complete, front-end can call radar API and PATCH with resultData. Thermometer: initialData has startLat/startLng, thresholdMeters; stop when far enough, call thermometer API, PATCH with result. Photo: initialData has photoType/photoId; on complete, note already has photoType so `unlockedPhotoTypes` derives from completedHints; photo-unlock API returns URL for display.
5. `PhotoPowerup`: `alreadyCompleted = unlockedPhotoTypes.has(photo.type)`; show "✓ Unlocked" and image. Prevent duplicate keys: `key={\`${photo.type}-${unlockedPhoto.photoId}\`}`; guard duplicate reveals in `handleRevealPhoto`.
