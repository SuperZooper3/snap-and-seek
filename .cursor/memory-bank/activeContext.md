...
- **Seeking UI:** `PowerupTabs` with folder-style tabs (Radar, Thermometer, Photo). When one hint is casting, other tabs disabled. Radar: reusable, result inline; Thermometer/Photo: per-target completed state. Components: `CastingTimer`, `RadarPowerup`, `ThermometerPowerup`, `PhotoPowerup`, `HintHistory`.
...
### Radar (as power-up)
- **Radar** is one of three power-up types. Hints flow: POST hints → casting timer → PATCH complete. When timer auto-completes, PATCH handler uses lat/lng/distanceMeters from `hint.note` (initialData) if `resultData` not provided, computes distance to hider's hiding photo, writes result to note.
- **Radar UI:** Distance stepper + Cast button always visible. Result shown inline below button: "Yes — you're within range." or "No — you're not within range." No separate "completed" view; button re-enables so user can scan again.
- **getLocation:** RadarPowerup uses `getLocation()` (debug-aware). Debug mode works for radar testing.
...
- **Power-ups flow:** Select target → choose tab (Radar/Thermometer/Photo) → start hint (POST hints) → CastingTimer runs → on complete, PATCH hint (resultData optional for radar; PATCH falls back to hint.note) → add to `completedHints`. Poll hints every 2s. Radar reusable; Thermometer/Photo per-target completed.
- **Radar (power-up):** Distance stepper → Cast Radar → timer → result stored in hint note. Result inline: "Yes — you're within range." or "No — you're not within range." Reusable; button re-enables after scan. Uses `getLocation()` (debug-aware). Thermometer: set start → Start (timer) → move ≥ threshold → when cast time done, click Stop (disabled until distance met) → hotter/colder/neutral. Uses `getLocation()` (debug-aware). Reusable. Result at bottom. Photo: Types with a photo show Unlock → casting → image. Types with "I don't have this option" show absence upfront.
