...
- `app/games/[gameId]/page.tsx` — server: game + players, join URL, zone; renders `GameActions` with zone. Passes zone (center_lat, center_lng, radius_meters or null) to actions.
- `app/games/[gameId]/GameActions.tsx` — client: copy join link, "Set game zone" / "Edit game zone", "Start game" (disabled until zone set + ≥2 players). Opens `GameZoneModal`; on start → `router.push(/games/[gameId]/zone)`.
...
- `app/games/[gameId]/zone/page.tsx` — server: fetches game + zone, renders header + `ZoneWithLocation` + footer with "Go to photo capture".
...
- `app/games/[gameId]/capture/page.tsx` — server: placeholder "Photo capture — coming soon" with back link to zone.
